...
let
  ...
  # Upstream is a uv workspace application (`package = false`), so the backend
  # is not pip-installable; it runs from source with its dependencies on
  # PYTHONPATH.
...
stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    ...
    wrap ${pythonEnv}/bin/gunicorn romm --add-flags "main:app" \
      ...
      --run 'export GUNICORN_CMD_ARGS="--bind=''${ROMM_HOST}:''${ROMM_PORT} --worker-class uvicorn_worker.UvicornWorker --workers ''${WEB_CONCURRENCY:-2} ''${GUNICORN_CMD_ARGS:-}"'
  ...
  meta = {
    ...
    changelog = "https://github.com/rommapp/romm/releases/tag/${version}";
